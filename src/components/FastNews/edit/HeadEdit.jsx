...
import { addElem } from "../../../config/fastnews";
...
const HeadEdit = () => {
  const { fastnews } = useSelector(addElem);
  const navigate = useNavigate();
  const { id } = useParams();
  const click = async () => {
    console.log(fastnews);
    await axios
      .patch(`https://api.agroxizmatlar.uz/information/${id}`, fastnews, {
        withCredentials: true,
      })
      .then((res) => {
        if (res.status == 200) {
          alert("malumot o'zgartirildi");
          navigate("/dash/fastnews");
        }
      });
  };
  return (
