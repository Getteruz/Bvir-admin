...
import { addElem } from "../../../config/aditional";
...
const HeadEdit = () => {
  ...
  const { aditional } = useSelector(addElem);
  const { id } = useParams();
  const click = async () => {
    await axios
      .patch(`https://api.agroxizmatlar.uz/default-page/${id}`, aditional, {
        withCredentials: true,
      })
      .then((res) => {
        if (res.status === 200) {
          alert("malumot o'zgartirildi");
          navigate("/dash/additional");
        }
      });
  };
  return (
