...
import { addElem } from "../../../config/statstic";
...
`;
const HeadEdit = () => {
  const { statistic } = useSelector(addElem);
  const navigate = useNavigate();
  const { id } = useParams();
  const click = async () => {
    try {
      await axios
        .patch(`https://api.agroxizmatlar.uz/statistic/${id}`, statistic, {
          withCredentials: true,
        })
        .then((res) => {
          if (res.status === 200) {
            alert("malumot o'zgartirildi");
            navigate("/dash/statistic");
          }
        });
    } catch (error) {
      console.log(error);
