...
import { addElem } from "../../../config/central.js";
...
const HeadPost = () => {
  ...
  const { central } = useSelector(addElem);
  const { id } = useParams();
  const click = async () => {
    await axios
      .patch(`https://api.agroxizmatlar.uz/management/${id}`, central, {
        withCredentials: true,
      })
      .then((res) => {
        if (res.status === 200) {
          alert("malumot o'zgartirildi");
          navigate("/dash/central");
        }
      });
  };
  return (
